...
- **Мультимодальные инструменты**: безопасный (на основе AST, без
  ...
  разделением по говорящим (`faster-whisper`) и анализ сцен видео + OCR
  (OpenCV) — всё на CPU, без PyTorch.
...
Образ включает аудио/видео-инструменты (`faster-whisper`, OpenCV), но
*не* `llama-cpp-python` — он общается с Ollama по обычному HTTP вместо
