...
OCR в `analyze_video` рассчитан на тот тип контента, который указан
в собственном описании инструмента — скринкасты, записи лекций,
видео со слайдами, где текст крупный и специально скомпонован для
восприятия. На сырой записи экрана терминала/IDE всё намного хуже:
мелкий моноширинный шрифт, сильное сжатие видео ровно в тех местах,
где этот инструмент и делает захват кадра (на смене сцены), плюс
псевдографика и спецсимволы, на которых OCR-модели никогда не
обучались. Увеличение и бинаризация кадра перед OCR не даёт надёжного
улучшения, если сжатие уже уничтожило детали — это проверено на
практике, а не предположение. Если тебе важно, чтобы текст с экрана
терминала/кода распознавался хорошо, записывай видео с более крупным
шрифтом и/или в более высоком разрешении — именно это реально
помогает, а не постобработка кадра после факта.
